...
The relay's compose file is committed at [`relay/docker-compose.yml`](relay/docker-compose.yml). The pubkey is generated by `hbbs` on first start and stored at `~/rustdesk-server/data/id_ed25519.pub`. **As long as `~/rustdesk-server/data/` is backed up**, restoring is:

```bash
mkdir -p ~/rustdesk-server && cd ~/rustdesk-server
# copy relay/docker-compose.yml from this repo to ./docker-compose.yml
# restore ./data/ from your backup (this is the part not in git)
docker compose up -d
```
...
├── .gitignore                 # blocks relay/data/ (private key) from ever being committed
└── relay/
    └── docker-compose.yml     # the relay stack, for recovery (data/ backed up separately)
```
